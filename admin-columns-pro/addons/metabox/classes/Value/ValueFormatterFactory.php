...
class ValueFormatterFactory
{
    ...
    public function create(FormatterCollection $formatters, Field\Field $field, Config $config): FormatterCollection
    {
        switch ($field->get_type()) {
            ...
            case MetaboxFieldTypes::SWITCH:
                return $formatters->add(new AC\Formatter\YesIcon());
            ...
            case MetaboxFieldTypes::FILE_UPLOAD:
                if ($config->get('file_display') === 'preview') {
                    ...
                    $inner = new Value\Formatter\FilePreview($size);
                } else {
                    $inner = new Value\Formatter\FileLink((string)$config->get('file_link_to', ''));
                }

                return $formatters->prepend(
                    new Aggregate(new FormatterCollection([new AC\Formatter\ArrayToCollection(), $inner]))
                );
