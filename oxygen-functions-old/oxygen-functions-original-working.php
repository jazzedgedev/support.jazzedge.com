...
function je_return_active_member() {
    ...
    $memberships = [
        ...
        'JA_YEAR_LSN_ONLY', 'JA_YEAR_CLASSES_ONLY', 'JA_YEAR_PREMIER', 'JA_MONTHLY_STUDIO_DMP'
    ];
          ...
          echo '<div class="'.esc_attr($cell_classes).'">';
            ...
            if ($has) {
              ...
              foreach ($byDay[$dkey] as $ev) {
                ...
                echo '<li class="'.esc_attr($classes).'"><a title="'.esc_attr($hover).'" href="'.esc_url(get_permalink($ev)).'"><span class="t">'.
...
add_shortcode('je_events_list', function($atts){
  ...
  $start  = new DateTimeImmutable('@'.$now_ts);
  $end    = $start->modify('+365 days');
  ...
  foreach ($events as $ev) {
    ...
    if (!$dt) continue;
    $ts = $dt->getTimestamp();
    if ($ts >= $now_ts) $items[] = ['id'=>$ev->ID,'ts'=>$ts];
  }
  ...
  if (!$css){ $css=true; ?>
    <style id="je-events-list-css">
      ...
      .je-events-item{margin:10px 0;padding:10px 12px;border:1px solid #e6e6e6;border-radius:12px;background:#fff}
      .je-events-item a{text-decoration:none;color:#0a5d6c}
      .je-events-item a:hover{text-decoration:underline;color:#f04e23}
      .je-time{font-weight:800;margin-right:8px}
      .je-events-item[class*="et-"]{--et-bg:#f5f7fa;--et-accent:#6b767b;background:var(--et-bg)}
      .je-events-item[class*="et-"] .je-time{color:var(--et-accent)}
      .je-events-item.et-community-call{--et-bg:#e8f7ee;--et-accent:#1e7f41}
      .je-events-item.et-special{--et-bg:#efeaff;--et-accent:#6b4fd8}
      .je-events-item.et-coaching{--et-bg:#e9f3ff;--et-accent:#004555}
      .je-events-item.et-class{--et-bg:#fff1ec;--et-accent:#f04e23}
      .je-events-item.et-workshop{--et-bg:#fffbe6;--et-accent:#c58a00}
      .je-events-item.et-webinar{--et-bg:#eef6ff;--et-accent:#0a5d6c}
      .je-events-item.et-office-hours{--et-bg:#f0f9ff;--et-accent:#0b7285}
      .je-events-item.studio{border-left:4px solid #004555}
      .je-events-item.premier{border-left:4px solid #f04e23}
      .je-events-item.is-special{box-shadow:0 0 0 2px #efeaff inset}
    </style>
  <?php }
  ...
  if ($items){
    foreach ($items as $it){
      ...
      echo '<li class="'.esc_attr($cls).'"><span class="je-time">'.
           esc_html(wp_date('D, M j • g:i a', $it['ts'])).
           '</span> <a href="'.esc_url(get_permalink($pid)).'">'.
