...
class InnerBlocks
{
    ...
    public function renderProductCarousel1($attributes, $content, $block): string
    {
        $lastChanged = $attributes['last_changed'] ?? '';
        $clientId = Arr::get($attributes, 'wp_client_id', '');
        $blockId = $clientId ? 'fct_product_carousel_client_' . $clientId : '';
        ...
        if ($blockId) {
            ...
            if ($needsReset || true) {
                $raw_block_markup = $block->parsed_block['blockName'] ? serialize_block($block->parsed_block) : '';
                $newTransient = [
                    'set_at'       => gmdate('Y-m-d H:i:s'),
                    'last_changed' => $lastChanged,
                    'markup'       => $raw_block_markup,
        ...
        // Carousel config
        $slidesToShow = Arr::get($attributes, 'carouselSettings.slidesToShow', 3);
        $infinite = Arr::get($attributes, 'carouselSettings.infinite', 'no');
        $arrows = Arr::get($attributes, 'carouselSettings.arrows', 'no');
        $dots = Arr::get($attributes, 'carouselSettings.dots', 'no');

        // Duplicate products if infinite and fewer than slidesToShow
        $carouselProducts = $products->all();
        if ($infinite && count($carouselProducts) < $slidesToShow) {
            $i = 0;
            while (count($carouselProducts) < $slidesToShow) {
                $carouselProducts[] = $products[$i % count($products)];
                $i++;
            }
        }

        $totalPages = ceil(count($carouselProducts) / $slidesToShow);

        $output = '<div class="fct-product-gallery-wrapper">';
        $output .= '<div class="fct-product-gallery-carousel editor-carousel-preview swiper" style="--fct-slides-per-view:' . esc_attr($slidesToShow) . ';">';
        $output .= '<div class="editor-carousel-track swiper-wrapper">';

        foreach ($carouselProducts as $index => $product) {
            $output .= '<div class="editor-carousel-slide swiper-slide">';
            ob_start();
            (new \FluentCart\App\Services\Renderer\ProductCardRender($product))->render();
            $output .= ob_get_clean();
            $output .= '</div>';
        }
        ...
        if ($arrows) {
            $output .= '<button class="editor-carousel-nav prev swiper-button-prev" aria-hidden="true">‹</button>';
            $output .= '<button class="editor-carousel-nav next swiper-button-next" aria-hidden="true">›</button>';
        }

        if ($dots) {
            $output .= '<div class="editor-carousel-pagination swiper-pagination">';
            for ($i = 0; $i < $totalPages; $i++) {
                $activeClass = $i === 0 ? 'is-active' : '';
                $output .= '<span class="editor-carousel-dot swiper-pagination-bullet ' . esc_attr($activeClass) . '"></span>';
            }
            $output .= '</div>';
        }
        ...
        $settings = json_encode([
            ...
            'infinite' => $infinite ? 'yes' : 'no',
            'arrows' => $arrows ? 'yes' : 'no',
            'dots' => $dots ? 'yes' : 'no'
        ]);
        $output .= '<div class="fct-carousel-settings" style="display:none" data-carousel-settings=\'' . $settings . '\'></div>';
    ...
    public function renderProductCarousel($attributes, $content, $block): string
    {
        $lastChanged = $attributes['last_changed'] ?? '';
        $clientId = Arr::get($attributes, 'wp_client_id', '');
        $blockId = $clientId ? 'fct_product_carousel_client_' . $clientId : '';
        ...
        $slidesToShow = Arr::get($attributes, 'slidesToShow', 3);
        $infinite = Arr::get($attributes, 'infinite', 'no');
        $arrows = Arr::get($attributes, 'arrows', 'no');
        $dots = Arr::get($attributes, 'dots', 'no');
        ...
        $output .= '<div class="swiper-slide fct-product-card" data-fct-product-card>';
        ...
        foreach ($block->inner_blocks as $inner_block) {
            if (isset($inner_block->parsed_block)) {
                $instance = new \WP_Block(
                    ...
                    array_merge(
                        $block->context,
                        ['fluent_cart_current_product' => $product]
                    )
                );
    ...
    public function renderProductContainer($attributes, $content, $block): string
    {
        ...
        wp_enqueue_style(
            'swiper-css',
            Vite::getAssetUrl('public/product-carousel/swiper/swiper-bundle.min.css'),
        );

        wp_enqueue_script(
            'swiper-js',
            Vite::getAssetUrl('public/product-carousel/swiper/swiper-bundle.min.js'),
            [],
            null,
            true
        );
        ...
        Vite::enqueueStyle(
                'fluentcart-product-carousel-css',
                'public/product-carousel/style/product-carousel.scss',
        );

        Vite::enqueueScript(
                'fluentcart-product-carousel-js',
                'public/product-carousel/product-carousel.js',
                []
        );
