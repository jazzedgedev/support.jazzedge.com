<?php
/**
 * Repository: apb_sides_characters.
 *
 ...
 * CRUD for characters.
 */
class APB_Sides_Character_Repo {

	private function table(): string {
		global $wpdb;
		return $wpdb->prefix . 'apb_sides_characters';
	}
	...
	public function get_all( array $args = array() ): array {
		...
		if ( ! empty( $args['search'] ) ) {
			$where[]   = 'name LIKE %s';
			$prepare[] = '%' . $wpdb->esc_like( (string) $args['search'] ) . '%';
		}

		$orderby = isset( $args['orderby'] ) ? sanitize_key( (string) $args['orderby'] ) : 'id';
		if ( ! in_array( $orderby, array( 'id', 'created_at', 'name', 'status' ), true ) ) {
			$orderby = 'id';
		}
	 ...
	 * @return array<int, array<string, mixed>>
	 */
	public function get_by_script_id( int $script_id ): array {
		global $wpdb;
		$table = $this->table();
		$rows  = $wpdb->get_results(
			$wpdb->prepare( "SELECT * FROM {$table} WHERE script_id = %d ORDER BY name ASC", $script_id ),
			ARRAY_A
		);
		return is_array( $rows ) ? $rows : array();
	}
