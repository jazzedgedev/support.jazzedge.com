            ...
            return $html;
				...
				if ($events) {
					...
					foreach ($events as $event) {
						...
						// Start datetime string (ACF). Prefer your per-user tz helper if available.
						$start_raw = get_post_meta($event_id, 'je_event_start', true);
						$user_tz   = isset($_SESSION['user-timezone']) ? $_SESSION['user-timezone'] : '';
						if (function_exists('format_timezone') && !empty($user_tz)) {
							$date_str = format_timezone($start_raw, $user_tz);
						} else {
