<?php
/*
Plugin Name: Wmyette Shortcodes - BACKUP
...
class Wmyette_Shortcodes {
	...
	//	add_shortcode( 'facet_return_coaching_thumbnail', array( $this, 'facet_return_coaching_thumbnail' ) );
	//	add_shortcode( 'facet_return_class_thumbnail', array( $this, 'facet_return_class_thumbnail' ) );
		add_shortcode( 'facet_return_replay_thumbnail', array( $this, 'facet_return_replay_thumbnail' ) );
	//	add_shortcode( 'facet_academy_message', array( $this, 'facet_academy_message' ) );
	//	add_shortcode( 'facet_premier_message', array( $this, 'facet_premier_message' ) );
		...
		if ($return === 'user_id') {
			...
			$html = '<div style="margin-top: 6px"><select id="dynamic_select_lesson_collection" class="classic"> 
			  <option value="" selected>Choose a lesson collection</option>
				...
				foreach ($courses AS $course) {
					$permalink = get_permalink($course->ID);
				$html .= '<option value="'.$permalink.'">'.mb_strimwidth($course->post_title,0,32,"...").'</option>';
				}
			$html .= '</select></div>';
			return $html;
		} elseif ($return === 'lesson_collections_trimmed_willie_delete') {
        	$courses = $wpdb->get_results( "SELECT * FROM wp_posts WHERE post_type = 'course' ORDER BY post_title ASC " );
			$html = '<div style="margin-top: 6px"><table>';
				foreach ($courses AS $course) {
					$permalink = get_permalink($course->ID);
				$html .= '<tr><td>'.$permalink.'</td><td>'.$course->post_title.'</td></tr>';
				}
			$html .= '</table></div>';
			return $html;
		} elseif ($return === 'recent_class_replays') {
        	//$events = $wpdb->get_results( "SELECT event_id,title,date FROM academy_event_recordings ORDER BY date DESC LIMIT 5" );
			...
			if ($meta) {
				...
				foreach ($meta AS $m) {
					...
					$event_date = date ('F jS, Y g:i a', strtotime($m->meta_value));
...
" );
			...
			global $wpdb,$user_id;
			$q = $wpdb->get_results( "SELECT * FROM academy_favorites WHERE user_id = $user_id AND type != 'resource' $fav_type_search ORDER BY datetime DESC LIMIT 5" );
			$html = '<div class="pc_container">';
			$empty = TRUE;
			foreach ($q AS $fav) {
				$empty = FALSE;
				...
				$html .= '<a href="'.$permalink.'" class="pc_container-recents">'.$icon.' '.$title.'
					</a>';
				}
			$html .= '</div>';
			if ($empty) {
			...
			global $wpdb,$user_id;
			$q = $wpdb->get_results( "SELECT * FROM academy_favorites WHERE user_id = $user_id AND type = 'resource' AND link_type != '' AND course_or_lesson_id > 0 ORDER BY datetime DESC LIMIT 5" );
			$html = '<div class="pc_container">';
			$empty = TRUE;
			foreach ($q AS $fav) {
				$empty = FALSE;
				...
				/*
				fa-sharp fa-solid fa-file-zipper = zip
				fa-sharp fa-solid fa-sliders-h = midi
				fa-sharp fa-solid fa-volume = audio
				fa-sharp fa-solid fa-music = sheet mx
				*/
				$html .= '<a href="'.$resource_link.'" aria-label="Resource Type: '.$resource_type.'" data-microtip-position="top" role="tooltip" class="pc_container-recents" ><i class="'.$font_awesome_class.'"></i>'.$lesson_title.'
					</a>';
			}
			$html .= '</div>';
			if ($empty) {
			...
			/*
			$html .= ' <div class="pc_image-container-20"><i class="fa-solid fa-circle-1 max_menu_numbers"></i></div>
					<div class="pc_description-container-80 padding-top-5">
						<h2><a href="'.$forum_link.'" target="_blank">Introduce Yourself</a></h2>
						<p class="pc_event_subtitle">Introduce yourself to other students</p>
					</div>';
			*/
			$html .= '
			...
			$tips = get_posts( $args );
			//echo '<pre><h2>$tips</h2>'; print_r($tips); echo '</pre>';
			...
			if ( $pc_events ) {
				...
				foreach ( $pc_events as $pc_event ) {
					$pc_event_date = get_post_meta( $pc_event->ID, 'premier_class_release_date', true );
					$premier_live_class_date = get_post_meta( $pc_event->ID, 'premier_live_class_date', true );
					...
					if (!empty($premier_live_class_date)) {
						...
						$pc_event_date_utz = format_timezone($premier_live_class_date,$user_timezone,'string');
					} else {
						$permalink = get_the_permalink($pc_event->ID);
						$pc_event_date_utz = format_timezone($pc_event_date,$user_timezone,'string');
					}
					$html .= ' <div class="pc_image-container-20">'.ja_teacher_headshot($premier_course_teacher).'</div>
					<div class="pc_description-container-80" style="padding-top: 4px;">
						<h2><a href="'.$permalink.'">'.$pc_event->post_title.'</a></h2>
	...
	function je_return_facet_listing_template( $atts , $content = NULL ) { 
		$atts = shortcode_atts(
			array(
				'template' => 'lessons',	
			), $atts, 'je_return_facet_listing_template'
		);
	...
	function facet_truncate_description( $atts , $content = NULL ) { 
		$atts = shortcode_atts(
			array(
				'text' => '',			
			), $atts, 'facet_truncate_description'
		);
		$text = $atts['text'];
	...
	/*
	function facet_return_coaching_thumbnail() { 
		$post_id = get_the_ID();
		$teacher = get_field( "teacher", $post_id );
        if ($teacher == 'willie') {
	...
	function facet_return_class_thumbnail() { 
		$post_id = get_the_ID();
...
/*
	function facet_academy_message() { 
		$level = get_field( "required_membership_level" );
		if (memb_hasMembership('ACADEMY_ACADEMY') == TRUE && $level <= 2 || memb_hasMembership('ACADEMY_ACADEMY_NC') == TRUE && $level <= 2) { 
			return '<div class="academy-access-message">You have access to this session.</div>'; 
		}	else { return; }
	}

	function facet_premier_message() { 
		$level = get_field( "required_membership_level" );
		if (memb_hasMembership('ACADEMY_PREMIER') == TRUE && $level == 3) { 
			return '<div class="academy-access-message">This is a PREMIER/COACHING-level session.</div>'; 
		}	else { return; }
	}
*/
	...
	function je_return_lesson_data( $atts , $content = NULL ) { 
		$atts = shortcode_atts(
			array(
				'return' => 'lesson_title',	
				'lesson_id' => 0,		
			), $atts, 'je_return_lesson_data'
		);
		if ($atts['lesson_id'] > 0) { $lesson_id = intval($atts['lesson_id']); }
		else { $lesson_id = get_post_meta( get_the_ID(), 'lesson_id', true ); }
		if ($lesson_id == 0) { return '<div style="text-align: center; width:100%;">No lesson found.</div>'; }
		global $wpdb;
		$q = $wpdb->get_row( "SELECT * FROM academy_lessons WHERE ID = $lesson_id " );
		$course_id = intval($q->course_id);
		if ($atts['return'] == 'lesson_title') { $return = stripslashes(htmlspecialchars($q->lesson_title)); }
		if ($atts['return'] == 'course_id') { $return = $course_id; }
		if ($atts['return'] == 'lesson_description') { $return = stripslashes(htmlspecialchars($q->lesson_description)); }
		if ($atts['return'] == 'course_title') { 
			$return = stripslashes(htmlspecialchars($wpdb->get_var( "SELECT course_title FROM academy_courses WHERE ID = $course_id " )));
		}
		/*
		if ($atts['return'] == 'course_title') { 
			$return = stripslashes(htmlspecialchars($wpdb->get_var( "SELECT course_title FROM academy_courses WHERE ID = $course_id " )));
	...
	function je_return_lesson_chapters( $atts , $content = NULL ) { 
		$atts = shortcode_atts(
			array(
				'format' => 'well',
			), $atts, 'je_return_lesson_chapters'
		);
		global $wpdb;
		...
		if ($lesson_id == 0) { return 'No lesson found.'; }
		$chapters = $wpdb->get_results( "SELECT * FROM academy_chapters WHERE lesson_id = $lesson_id ORDER BY menu_order ASC" );
		if (empty($chapters)) { return '<div style="text-align: center; width:100%; ">No chapters found.</div>'; }
		// ********** SHOW IN WELL
		if ($atts['format'] == 'well') {
			...
			foreach ($chapters AS $chapter) {
				...
				<!--/.lesson-->';
				$counter++;
				$total_lesson_duration = $total_lesson_duration + $chapter->duration;
			}
			$return .= '			

			  <li style="padding:30px 0px; font-size:10pt; text-align:center;">Total Lesson Duration = '.format_time($total_lesson_duration).'</li>
			</ul>
			<!-- /.list-items -->
			</div>
			<!-- /.well -->';
	...
	function je_return_lesson_resources( $atts , $content = NULL ) { 
		//if (je_check_free_access('resources') == 'maxed') { return '<div style="text-align: left; ">You have viewed (3) resources. Please <a href="/upgrade">upgrade your membership</a> to download more resources.</div>'; }
		$atts = shortcode_atts(
			array(
			), $atts, 'je_return_lesson_resources'
		);
		//$lesson_id = je_return_id_from_slug($_GET['l'],'lesson');
		$access = FALSE;
		$post_id = get_the_ID();
		$lesson_id = get_post_meta( $post_id, 'lesson_id', true );
		...
		if (is_array($tags)) {
			if (in_array('Song Lesson',$tags)) { $song_level_access = TRUE; }
			if (in_array('Academy Success',$tags)) { $song_level_access = TRUE; }
		} else { }
		...
		/* need to fix the rental access to resources before going live
		if ($rental_status == 'expired') {
			return '<p>Your access to these resources has expired. <a href="https://jazzedge.academy/upgrade">Upgrade</a> your membership to gain access to the sheet music and resources.</p>';
		}
		*/
	...
	function je_return_lesson_resources_new( $atts , $content = NULL ) { 
		...
		$atts = shortcode_atts(
			array(
			), $atts, 'je_return_lesson_resources_new'
		);
		//$lesson_id = je_return_id_from_slug($_GET['l'],'lesson');
		...
		//$je_return_active_member = je_return_active_member();
		...
		/*
		if ($je_return_active_member == 'true') {
			if (ja_is_classes_only_member() === 'true') {
				$access = FALSE;
			} else {
				$access = TRUE;
			}
		} elseif ($academy_credit_access == 'true') {
			$access = TRUE;
		} else { return $no_access_message; }
		...
		*/
		...
		if ($je_has_lesson_access === 'true' && $user_membership_level_num > 1) { $access = TRUE; }
		if ($academy_credit_access === 'true') {	$access = TRUE;	} 
		if ($access === FALSE) {
			return $no_access_message; 
		}
	...
	function je_return_lesson_progress( $atts , $content = NULL ) { 
		$atts = shortcode_atts(
			array(
				'format' => 'percent',
				'lesson_id' => 0,
			), $atts, 'je_return_lesson_progress'
		);
		$format = $atts['format'];
		$post_id = get_the_ID();
		$lesson_id = get_post_meta( $post_id, 'lesson_id', true );
		$lesson_id = ($atts['lesson_id'] > 0) ? $atts['lesson_id'] : $lesson_id;
		if ($format == 'percent') { return je_return_lesson_progress_percentage($lesson_id,'percent'); }
		elseif ($format == 'full') { return je_return_lesson_progress_percentage($lesson_id,'full'); }
	}	
	...
	function je_return_chapter_data( $atts , $content = NULL ) { 
		$atts = shortcode_atts(
			...
			), $atts, 'je_return_chapter_data'
		);
		$chapter_slug = (isset($_GET['c'])) ? sanitize_text_field($_GET['c']) : '';
		$atts_return = (isset($atts['return'])) ? $atts['return'] : '';
		global $wpdb;
		$q = $wpdb->get_row( "SELECT * FROM academy_chapters WHERE slug = '$chapter_slug'" );
		if ($atts_return == 'chapter_title') { $return = (isset($q->chapter_title)) ? stripslashes($q->chapter_title) : ''; }
		if ($atts_return == 'vimeo_id') { 
			...
			} else { $vimeo_id = $q->vimeo_id; }
			$return = $vimeo_id;
		}
		if ($atts['return'] == 'duration') { $return = $q->duration; }
	...
	function je_return_course_data( $atts , $content = NULL ) { 
		$atts = shortcode_atts(
			array(
				'return' => 'course_title',	
				'course_id' => 0,		
			), $atts, 'je_return_course_data'
		);
		$chapter_slug = (isset($_GET['c'])) ? sanitize_text_field($_GET['c']) : '';

		if ($atts['course_id'] > 0) { $course_id = intval($atts['course_id']); }
		else { $course_id = je_return_id_from_slug($chapter_slug,'course'); }
		
		if ($course_id == 0) { return 'No course found.'; }
		global $wpdb;
		$q = $wpdb->get_row( "SELECT * FROM academy_courses WHERE ID = $course_id " );
		if ($atts['return'] == 'course_id') { $return = $q->ID; }
		if ($atts['return'] == 'course_description') { $return = $q->course_description; }
		if ($atts['return'] == 'course_title') { $return = $q->course_title; }
	...
	function je_return_course_lessons( $atts , $content = NULL ) { 
		...
		if ($atts['format'] == 'well') {
			...
			$return .= '			
			  ...
			  <li style="padding:30px 0px; font-size:10pt; text-align:center;">Total Collection Duration = '.format_time($total_lesson_duration).'</li>
			</ul>
			<!-- /.list-items -->
			</div>
			<!-- /.well -->';
	...
	function je_return_course_progress( $atts , $content = NULL ) { 
		$atts = shortcode_atts(
			array(
				'format' => 'percent',
				'course_id' => 0,
			), $atts, 'je_return_course_progress'
		);
		$format = $atts['format'];
		$post_id = get_the_ID();
		$course_id = get_post_meta( $post_id, 'course_id', true );
		$course_id = ($atts['course_id'] > 0) ? $atts['course_id'] : $course_id;
		if ($format == 'percent') { return je_return_course_progress_percentage($course_id,'percent'); }
		elseif ($format == 'full') { return je_return_course_progress_percentage($course_id,'full'); }
	}	
	...
	function je_return_path_progress( $atts , $content = NULL ) { 
		$atts = shortcode_atts(
			array(
				'format' => 'percent',
				'path_id' => 0,
			), $atts, 'je_return_path_progress'
		);
		$format = $atts['format'];
		$path_id = (isset($atts['path_id'])) ? $atts['path_id'] : get_the_ID();
		if (je_return_number_of_lessons_in_path($path_id) == 0) { return; }
		if ($format == 'percent') { return je_return_path_progress_percentage($path_id,'percent'); }
		elseif ($format == 'full') { return je_return_path_progress_percentage($path_id,'full'); }
	}	
	...
	function je_video_player( $atts , $content = NULL ) { 
		...
		$preroll = '';
		$postroll = '';
		// $vimeo_id = 0; // used for testing
		
		// Check ACADEMY_SONG Level
		$post_id = get_the_ID();
		$user_membership_level = je_return_membership_level();
		$tags = get_post_meta( $post_id, 'lesson_tags', true );
		$song_level_access = FALSE;
		...
		//$is_lesson_rented = je_return_rental_details('lesson_rented',$lesson_id);
		$user_id = get_current_user_id();
		if ($user_id == 2001) {
			// echo '<pre><h2>$tags</h2>'; print_r($tags); echo '</pre>';

// 			echo "song_level_access = $song_level_access<br>";
// 			if ($song_level_access == TRUE) { echo 'it is TRUE'; }
// 			echo "rental_status = $rental_status";
		}
		$rental_status = je_check_rental_access();
		...
		if (!empty($tags)) {
			/*
			if ($song_level_access != TRUE && $user_membership_level == 'ACADEMY_SONG') {
				return '<div class="video_no_access"><p>You do not have access to this lesson with your current membership level. <a href="/upgrade" class="hover-black">Click here for upgrade options</a>.</p></div>';
			}
			*/
			if ($rental_status == 'expired' && !in_array('Song Lesson',$tags) && $user_membership_level == 'ACADEMY_SONG') {
				return '<div class="video_no_access"><p>You do not have access to this lesson with your current membership level. <a href="/signup" class="hover-black ">Click here for upgrade options</a>. If you are a member, <strong>make sure you are logged in</strong> to your account.</p></div>';
			}
		}		
//		$return = do_shortcode( "[fvplayer src='https://vimeo.com/$vimeo_id' startend='00:00-00:05']" );
        return $return;
	}	
	...
	<table class='rg-table zebra' summary='Hed'>
		...
		foreach ($subscriptions AS $subscription) {
			...
			$billing_amount = money_format('%.2n', $subscription['BillingAmt']);
			...
			";
				...
				if (in_array($pid,$academy_pids)) {
					...
					$return .= "<td><a href='https://myjazzedge.com/?memb_autologin=yes&auth_key=9JeYjtLTtj3G&Id=$contact_id&Email=$user_email&redir=/billing' target='_blank'>Cancel Membership</a></td>";
				}
			} elseif ($pid == 62293) {
				echo "<tr>
						<td>1-Year</td>
		...
		$return .= '</tbody></table></div>';
			...
			if (memb_hasAnyTags(array(7754))) { // HSP Child Tag
			...
			if (memb_hasAnyTags(array(7746))) { // HSBC Tag
			...
			if (memb_hasAnyTags(array(9661)) && empty($academy_expiration_date)) { // ACADEMY_FREE
		...
		// check for 1x purchases (gonna use this??)
	//	$return .= je_get_one_time_memberships();
		return $return;
	}
	...
	function je_cancel_academy_subscription( $atts , $content = NULL ) { 
		...
		// 9745	Academy Cancellation Processing
		if ($step == 1) {
			include( '/nas/content/live/jazzacademy/willie/infusion_connect.php' );
		...
		if ($step == 3) {
			...
			// UPDATE EXPIRATION AND CANCEL TAGS
			/* Product IDs
				62241	ACADEMY_PREMIER	(cancel tag: 9739 - CANCEL_JA_PREMIER)
				62239	ACADEMY_ACADEMY	(cancel tag: 9737 - CANCEL_JA_ACADEMY)
				62237	ACADEMY_SONG	(cancel tag: 9735 - CANCEL_JA_SONG)
				**NEW**
				62334	JA_YEAR_STUDIO  // 	tag: 9962	CANCEL_JA_YEAR_STUDIO
				62332	JA_MONTHLY_STUDIO // tag: 9960	CANCEL_JA_MONTHLY_STUDIO
				62285	JA_MONTHLY_PREMIER // tag: 9837	CANCEL_JA_MONTHLY_PREMIER
				62293	JA_YEAR_PREMIER // tag: 9964	CANCEL_JA_YEAR_PREMIER
			*/
			
			// JA_YEAR_STUDIO
			if ($pid == 62334) { $cancel_tag_id = 9962; }
			
			// JA_MONTHLY_STUDIO
			if ($pid == 62332) { $cancel_tag_id = 9960; }
			
			// JA_MONTHLY_PREMIER
			if ($pid == 62285) { $cancel_tag_id = 9837; }
			
			// JA_YEAR_PREMIER
			if ($pid == 62293) { $cancel_tag_id = 9964; }
			
			
			// ****** OLD MEMBERSHIPS ****** //
			// 9978	CANCEL_JA_MONTHLY_CLASSES_ONLY
			if ($pid == 62313) { $cancel_tag_id = 9978; }
			
			// 9980	CANCEL_JA_YEAR_LSN_CLASSES
			if ($pid == 62291) { $cancel_tag_id = 9980; }
			
			// Premier
			if ($pid == 62241) { $cancel_tag_id = 9739; }
			// Academy
			if ($pid == 62239) { $cancel_tag_id = 9737; }
			
			// Academy NC
			if ($pid == 62257) { $cancel_tag_id = 9811; }
			
			
			//Song
			if ($pid == 62237) { $cancel_tag_id = 9735; }
			
			// JA_MONTHLY_LSN_CLASSES
			if ($pid == 62283) { $cancel_tag_id = 9831; }
			
			// JA_MONTHLY_LSN_COACHING
			if ($pid == 62281) { $cancel_tag_id = 9833; }
			
			// JA_MONTHLY_LSN_ONLY
			if ($pid == 62279) { $cancel_tag_id = 9835; }
			
			// JA_MONTHLY_PREMIER
			if ($pid == 62285) { $cancel_tag_id = 9837; }
			
			// Set Cancel Tag - this starts the cancellation campaign in Keap
			memb_setTags(array($cancel_tag_id));
		
			// Set Academy Expiration Date in Keap
			...
			if (empty($found)) { $wpdb->insert('academy_cancellations',$data,$format); }
	...
	function je_list_payments( $atts, $content = null ) {
		...
		if (memb_hasAnyTags(array(9757))) { // 	9757	Academy Free Month Applied
	...
	<table class='rg-table zebra' summary='Hed'>
		...
		krsort($payments);
			foreach ($payments AS $payment) {
				$amount = money_format('%.2n', $payment['PayAmt']);
				$total_amount += $amount;
