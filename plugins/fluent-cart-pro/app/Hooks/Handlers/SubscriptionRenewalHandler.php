...
class SubscriptionRenewalHandler
{
    ...
    public function handleSubscriptionReactivationRedirect($data = [])
    {
        ...
        $item = ProductItemService::getItem([
        ...
        $paymentType = Arr::get($variation->other_info, 'payment_type', $variation->payment_type);

        if (!$variation || !$product || Arr::get($variation->other_info, 'repeat_interval') != $subscription->billing_interval || $paymentType != 'subscription') {
        ...
        if (!$isCustom && !$variation->canPurchase() && !$requiredBillTimes) {
        ...
        if($isCustom) {
            Arr::set($newItem, 'post_title', $variation->post_title);
            Arr::set($newItem, 'variation_type', $variation->variation_type);
        } else {
        ...
        $instantCart->checkout_data = array_merge(
            $instantCart->checkout_data,
            $instantCart->addDraftCreatedActions([
                'fluent_cart/subscription_reactivation_order_created',
            ]),
            $instantCart->addSuccessActions([
                'fluent_cart/subscription_renewal_cart_completed',
            ]),
            $instantCart->addCartNotices([
                [
                    'id'      => 'renewal_notice',
                    'type'    => 'info',
                    'content' => sprintf(
